...
latent_1 = 4 + sin(time_bins);
latent_2 = time_bins * 1;
...
latent_1_nodes = randsample(1:num_nodes, 20);
latent_2_nodes = randsample(1:num_nodes, 20);

sim_spike_matrix(latent_1_nodes, :) = ... 
    sim_spike_matrix(latent_1_nodes, :) + latent_1;

sim_spike_matrix(latent_2_nodes, :) = ... 
    sim_spike_matrix(latent_2_nodes, :) + latent_2;

figure;
imagesc(sim_spike_matrix)


% Correlation 
distMatrix = corr(sim_spike_matrix');
% Dynamic time warping (TODO)
...
subplot(1, 2, 2)
imagesc(sim_spike_matrix(outperm, :))
title('Sorted order')
