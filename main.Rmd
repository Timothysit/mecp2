...
h <- h5.read.spikes("/media/timothysit/phts2/The_Mecp2_Project/feature_extraction/R_analysis/mecp2_spikes/UN_12_09_17-6A_DIV14.h5")
fourplot(h)
```
# Burst analysis 
...
mapAnalyse <- function(s, electrodeMap, measure = 'burst'){
  ...
  dim(spikeMap) <- c(8, 8)
  spikeMap <- t(spikeMap)
  resultMap <- log(spikeMap) # log scale 
  }
  ...
  scale_fill_gradientn(name="log(# Spikes)", na.value="gray", colours = c("white", viridis(256, option = "D")), values = c(0,0.5,1), limits = c(0, max(resultMap))) # use viridis but gradientn is easier for cotrolling other options
  
  # geom_text(aes(label = electrodeIndex)) # electrode index, shall work on this later (need to transpose electrodeIndex and flip)
  fileName <- substr(basename(s$file), 1,  nchar(basename(s$file))-3) # remove last three characters, which are ".h5"
  format <- ".png"
  ggsave(paste(fileName, format, sep = '')) 
}
